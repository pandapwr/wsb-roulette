...
def grab_options(ticker, cost):
    ...
    options_list.append([f'${ticker} {exp_date} {new_strike}{option}', f'${price}', qty, total_cost, iv, delta, theta, gamma, vega, rho])
...
print(tabulate(options_list, headers=["Option", "Price", "Qty", "Cost", "IV", "Delta", "Theta", "Gamma", "Vega", "Rho"]))

for i in range(0,len(options_list)):

    total = total + options_list[i][3]
